...
const mutations = {
    ...
    SET_MOST_ANIME_LIST_IN_LOCALSTORAGE(state: any, animeList: any) {
        localStorage.setItem("mostAnimeList", JSON.stringify(animeList));
    }
};
...
const actions = {
    async getMostAnimeList({commit}) {
        try {
            const lValue = localStorage.getItem("mostAnimeList")
            if (lValue) {
                commit('SET_MOST_ANIME_LIST', lValue);
            } else {
                ...
                commit('SET_MOST_ANIME_LIST_IN_LOCALSTORAGE', animeList)
            }
        } catch (error) {
            console.log(error);
        }
